<!doctype html>
<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>全网通营销管理后台-移动版</title>

    <link rel="stylesheet" type="text/css" href="/static/js/jquery.mobile-1.4.5.min.css">
    <script type="text/javascript" src="/static/js/jquery.min.js"></script>
    <script type="text/javascript" src="/static/js/jquery.mobile-1.4.5.min.js"></script>

</head>
<body>


<div data-role="tabs" id="tabs">
    <div data-role="navbar">
        <div data-role="header" data-position="fixed" data-tap-toggle="false" class="tab-list-content">
            <ul>
                <li><a href="#num" class="ui-btn-active">号码</a></li>
                <li><a href="#sum" >总和</a></li>
                <li><a href="#special" >特码</a></li>
            </ul>
        </div>

        <div id="num">

            <table data-role="table" data-mode="columntoggle:none" border="1" class="ui-responsive tab-data">
                <thead>
                <!--                <div data-role="header" data-position="fixed" data-tap-toggle="false">-->
                <tr>
                    <th class="chinese">期数</th>
                    <th>正一</th>
                    <th>正二</th>
                    <th>正三</th>
                    <th>正四</th>
                    <th>正五</th>
                    <th>正六</th>
                    <th>特码</th>

                </tr>
                <!--                </div>-->
                </thead>
                <tbody>
                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>
                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>
                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>
                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>
                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>
                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>

                <tr>
                    <td>2017036</td>
                    <td><span class="active-red">40</span>马</td>
                    <td><span class="active-blue">33</span>牛</td>
                    <td><span class="active-green">01</span>羊</td>
                    <td><span class="active-red">03</span>猴</td>
                    <td><span class="active-red">22</span>龙</td>
                    <td><span class="active-red">23</span>蛇</td>
                    <td><span class="active-red">32</span>鼠</td>

                </tr>
                </tbody>
            </table>
        </div>
        <div id="sum">


            <table data-role="table" data-mode="columntoggle:none" border="1" class="ui-responsive tab-data">
                <thead>
                <!--                <div data-role="header" data-position="fixed" data-tap-toggle="false">-->
                <tr>
                    <th class="chinese">期数</th>
                    <th>总数</th>
                    <th>单双</th>
                    <th>大小</th>
                    <th>七色波</th>
                </tr>
                <!--                </div>-->
                </thead>
                <tbody>
                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>187</td>
                    <td>单</td>
                    <td>大</td>
                    <td>红波</td>
                </tr>

                </tbody>
            </table>
        </div>
        <div id="special">


            <table data-role="table" data-mode="columntoggle:none" border="1" class="ui-responsive tab-data">
                <thead>
                <!--                <div data-role="header" data-position="fixed" data-tap-toggle="false">-->
                <tr>
                    <th class="chinese">期数</th>
                    <th>单双</th>
                    <th>大小</th>
                    <th>和单双</th>
                    <th>和大小</th>
                    <th>大小尾</th>
                </tr>
                <!--                </div>-->
                </thead>
                <tbody>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>

                <tr>
                    <td>2017036</td>
                    <td>双</td>
                    <td>大</td>
                    <td>和双</td>
                    <td>和大</td>
                    <td>尾小</td>
                </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>

</body>
</html>
<script>
    $(document).ready(function () {
        $(".tab-data").find("tr").css("background-color", "#ffffff");
        $(".tab-data").find("tr:even").css("background-color", "#f0f8ff");
    });
</script>

<style>
    .ui-tabs {
        padding: 0;

    }

    .ui-page-theme-a .ui-btn, html .ui-bar-a .ui-btn, html .ui-body-a .ui-btn, html body .ui-group-theme-a .ui-btn, html head + body .ui-btn.ui-btn-a, .ui-page-theme-a .ui-btn:visited, html .ui-bar-a .ui-btn:visited, html .ui-body-a .ui-btn:visited, html body .ui-group-theme-a .ui-btn:visited, html head + body .ui-btn.ui-btn-a:visited {
        border: 0;
        background-color: #ffffff;
    }

    .ui-page-theme-a .ui-btn.ui-btn-active {
        color: red;
        background-color: #FFFFFF;
        text-shadow: none;
        border-bottom: 2pt solid red;

    }

    .ui-page-theme-a .ui-btn:focus {
        box-shadow: none;
        border-bottom: 2pt solid red;
    }

    .ui-header, .ui-footer {
        border: 0;
        background-color: transparent;
    }

    .ui-page-theme-a {
        border: 0;
        background-color: transparent;
        font-family: . PingFang-SC-Regular;
        font-size: 10px;
        color: #6B6B6B;
        letter-spacing: 0;
        text-align: center;
    }

    .ui-table th, .ui-table td {
        padding: 5pt 4pt;
        text-align: center;
    }

    .ui-bar-a, .ui-page-theme-a .ui-bar-inherit, html .ui-bar-a .ui-bar-inherit, html .ui-body-a .ui-bar-inherit, html body .ui-group-theme-a .ui-bar-inherit {
        /*background: rgb(255,223,215);*/
    }

    .ui-footer .ui-bar-inherit .ui-footer-fixed .slideup {
        background-color: red;
    }

    #footer-table {
        background: rgba(255, 223, 215, 0.74);
        border-top: 1px solid #FF8C94;
    }

    /*.chinese {*/
        /*width: 16%*/
    /*}*/

    th {
        width: 12%;
        height: 16pt;
        border-right: 1pt solid #8F8F8F;
    }

    td {
        width: 12%;
        height: 16pt;
        border-right: 1pt solid #8F8F8F;
    }

    .active-red{
        color: #FFFFFF;
        background-color: #0000cc;
        border-radius: 50%;
        width: 15pt;
        height: 16pt;
    }

    .active-blue {
        color: #FFFFFF;
        background-color: #ff0000;
        border-radius: 50%;
        width: 15pt;
        height: 16pt;
    }

    .active-green {
        color: #FFFFFF;
        background-color: #00cc04;
        border-radius: 50%;
        width: 15pt;
        height: 16pt;
    }

    th:last-child{
         border: none;
     }

    td:last-child{
        border: none;
    }

    th:first-child{
        width: 16%
    }

    td:first-child{
        width: 16%
    }

</style>

